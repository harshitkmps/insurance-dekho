...
export default function CustomizedHook(props) {
  ...
  const SelectionList = field.options;
  ...
  const {
    ...
    onChange: function handleChange(e, values, action, selected) {
      let value = field.value || [];
      if(action == 'selectOption'){
        value.push(selected.option)
      }else if(action == 'removeOption'){
        value = value.filter(i => i.value != selected.option.value)
      }
  ...
  return (
    <Root>
      <div {...getRootProps()}>
        <Label {...getInputLabelProps()}>Countries to Visit</Label>
        <div
          style={{
            flexwrap: "wrap",
            display: "flex",
        ...
        >
          <InputWrapper ref={setAnchorEl} className={focused ? "focused" : ""}>
            ...
            <input {...getInputProps()} disabled={field.disabled} />
          </InputWrapper>
        </div>
      </div>
