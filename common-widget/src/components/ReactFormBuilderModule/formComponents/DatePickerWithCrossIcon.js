...
export default function (props) {
  ...
  let handleCross=(value)=>{
    ...
    if (field.onClick) {
      field.onClick(value, step, (newstep) => {
        if (newstep) {
          dispatch({
            type: "UPDATE_STEP",
            payload: { step: newstep },
  ...
  return (<div onClick={()=>{field.date=null}}>
    {!field.toShow && <FormControl error={field.error} className="fb_selectWlabel  " fullWidth={true}>
    {field.title ? (
      <Grid container className="fb_row">
        <Grid item xs={4} sm={3} className="leftLabel">
          <FormLabel>{field.title}</FormLabel>
        </Grid>
        <Grid className="fb_datepick " item xs={8} sm={8}>{commonDate}</Grid>
      </Grid>
    ) : (
      commonDate
    )}
...
class CustomDatePicker extends React.Component {
  ...
  select(fieldName, fieldValue) {
    ...
    var mmm = String(newDate.getMonth() + 1).padStart(2, "0");
    var yyyyy = newDate.getFullYear();
    this.props.onChange(yyyyy + "-" + mmm + "-" + ddd);
  }
    ...
    return (
      !toShow && <div>
        ...
        <TextField
          ...
          autoComplete="off"
          InputProps={{ disableUnderline: true, endAdornment: icon, readOnly: true }}
        ></TextField>
        ...
        {toShow !== undefined && toShow !== null && (
      <IconButton edge="end" style={{ float: 'right', paddingTop: '-20px' }} onClick={() => this.props.onclick("samaltran")}>
        <ClearIcon />
      </IconButton>
    )}
