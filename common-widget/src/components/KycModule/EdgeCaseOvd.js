...
import { BackStep, InprogressOkyc, InprogressCkyc,StartOvd} from '../../redux/actions/KycActions';
...
class EdgeCaseOvd extends React.Component{
  ...
  componentDidMount() {
    let kycState = this.props.kycState;
    let kycConfig = getSessionStorageItem('kycConfig');
    let ovd_fields = [];
    let ovd_doc = [];
    let ovd_field_slug = [];
    let ovd_doc_slug = [];
    get(kycState,'ovdFeild',[]).map((item)=>{
      if(!ovd_field_slug.includes(item.slug)){
        ovd_fields.push(item);
        ovd_field_slug.push(item.slug);
      }
    })
    get(kycState,'ovdOptions',[]).map((item)=>{
      if(!ovd_field_slug.includes(item.slug)){
        ovd_doc.push(item);
        ovd_doc_slug.push(item.slug);
      }
    })
    this.setState({
      kycConfig:kycConfig,
      pageFields:ovd_fields,
      pageImageField:ovd_doc
    });
  } 
  ...
  async submitHandler(e){
    ...
    if (this.validator.allValid()) {
      ...
      this.props.StartOvd({payload:{fields,is_id_module:this.props.kycState.is_id_module}});
        ...
        <form onSubmit={(e) => this.submitHandler(e)}>
          ...
          }
          {
            get(this,'state.pageFields',[]).map((item,index)=>{
              ...
              if(item.input_type == 'date'){
                return <div className="panInput" name="date_of_birth">
                  <CustomDatePicker
                    key={index}
                    id={`son`}
          ...
          <div className="nextButton">
            <button className="lineBtn" onClick={() => {
              if(get(this.props,'parentData.typeOfKyc') == 'ovd'){
                this.props.BackStep({ step: 'select-document' })
              } else{
                this.props.BackStep({ step: 'main' })
              }
            }}>Back</button>
            <button disabled={(this.props.kycState.loader.ovd_loader ? true : false)} className={`solidBtn ${(this.props.kycState.loader.ovd_loader ? "whiteCircle_m" : '')}`} type='submit'>Next</button>
          </div>
        </form>
      </div>
    )
...
const mapDispatchToProps = {
  BackStep, InprogressOkyc, InprogressCkyc, StartOvd
}
export default connect(mapStateToProps, mapDispatchToProps)(EdgeCaseOvd);